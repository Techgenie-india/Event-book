...
class WCFMmp_Gateway_Stripe_Split extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title = __('WCFM Stripe Split Pay', 'wc-multivendor-marketplace');
	...
	public function process_payment($order_id) {
						...
						try {
							...
							if (isset($this->charge['failure_message']) && empty($this->charge['failure_message'])) {
								...
								if (isset($this->charge->id)) {
									$order->update_meta_data('wcfmmp_stripe_split_pay_charge_id', $this->charge->id);
									...
									wcfmmp_log( sprintf( '#%s - payment processing complete via %s. Amount: %s', sprintf( '%06u', $withdrawal_id ), 'Stripe Split Pay', $distribution_info['commission'] . ' ' . $order->get_currency() ), 'info' );
								}
								$card_charged = true;
								...
								if ($this->debug)
								wcfm_log("Stripe Charge Error: " . $this->charge['failure_message']);
							...
							if ($this->debug)
								wcfm_log("Stripe Split Pay Error: " . $ex->getMessage());
				...
				if($card_charged && isset( $wcfmmp_stripe_split_pay_list['distribution_list'] ) && is_array( $wcfmmp_stripe_split_pay_list['distribution_list'] ) && count( $wcfmmp_stripe_split_pay_list['distribution_list'] ) > 0 ) {
					foreach( $wcfmmp_stripe_split_pay_list['distribution_list'] as $vendor_id => $distribution_info ) {
						...
						try {
							...
							if ($this->debug)
								wcfm_log("Error creating transfer record with Stripe: " . $ex->getMessage());
		...
		if ((is_array($all_success) && in_array( "false", $all_success )) || $this->vendor_disconnected) {
			$order->update_status( apply_filters('wcfmmp_stripe_split_pay_processing_order_status', 'processing') );
		} else {
	...
	protected function wcfmmp_stripe_split_pay_list( $order, $postData ) {
		...
		$args = array(
			...
			'transfer_group' => __('Split Pay #', 'wc-multivendor-marketplace') . $order->get_order_number(),
			'description'    => __('For order id #', 'wc-multivendor-marketplace') . $order->get_order_number()
		);
			...
			if ($product_id) {
				...
				if( $pvendor_id && ( $pvendor_id == $vendor_id ) ) {
					if( $WCFMmp->wcfmmp_vendor->is_vendor_deduct_discount( $vendor_id ) ) {
						$commission_amount += $WCFMmp->wcfmmp_commission->wcfmmp_get_order_item_commission( $order_id, $vendor_id, $product_id, $line_item->get_variation_id(), $line_item->get_total(), $line_item->get_quantity() );
					} else {
						$commission_amount += $WCFMmp->wcfmmp_commission->wcfmmp_get_order_item_commission( $order_id, $vendor_id, $product_id, $line_item->get_variation_id(), $line_item->get_subtotal(), $line_item->get_quantity() );
					}
					if( $WCFMmp->wcfmmp_vendor->is_vendor_get_tax( $vendor_id ) ) {
						$commission_amount = $commission_amount + $line_item->get_total_tax();
					}
				}
			}
		}
		...
		if( $get_shipping = $WCFMmp->wcfmmp_vendor->is_vendor_get_shipping( $vendor_id ) ) {
			...
			foreach ($shipping_items as $shipping_item_id => $shipping_item) {
				...
				if( $shipping_vendor_id > 0 && ( $shipping_vendor_id == $vendor_id ) ) {
					$commission_amount = $commission_amount + $order_item_shipping->get_total();
					if( $WCFMmp->wcfmmp_vendor->is_vendor_get_tax( $vendor_id ) ) {
						$commission_amount += $order_item_shipping->get_total_tax();
					}
